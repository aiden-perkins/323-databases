...
class CommandLogger(monitoring.CommandListener):
    def started(self, event):
        log.debug(
            'Command {0.command_name} with request id {0.request_id} started on server {0.connection_id}'.format(event)
        )

    def succeeded(self, event):
        log.debug('Command {0.command_name} with request id {0.request_id} on server {0.connection_id}'
